...
- mathhub.io 는 **wrangler 직접 업로드**로 배포된다(git 연동 아님). 프로덕션 브랜치 = `main`.
- **배포는 반드시 `bash scripts/deploy.sh` 로.** 이유: `wrangler pages deploy .` 는 디렉터리 전체를 올리고 `.gitignore`/`.assetsignore` 를 무시 → 그냥 올리면 `docs/`(내부 전략·보안 메모)가 공개됨. 스크립트가 `docs/`·`scripts/` 를 잠시 빼고 배포 후 복원하며 `--branch main`(프로덕션)으로 보낸다.
- wrangler 인증: `npx wrangler login` 1회(브라우저 OAuth). 비대화형이면 `CLOUDFLARE_API_TOKEN`.
- **2026-06-12 배포 완료·검증**: mathhub.io 에서 신규 풀이 2개 200, QAS PDF 404, `/docs/*` 404, 연습문제 200, qas 탭 0 확인.
